...
io.sockets.on("connection", (socket) => {
    ...
    socket.on("search", async (data, callback) => {
        try {
            ...
            objects = await CityModel.find(querySearch).limit(10000)
            total = await CityModel.countDocuments(querySearch)
...
//             objects = await CityModel.find({ $or: querySearch }).limit(10000)
